...
| Title | `XMP-dc:Title` |
| Caption | `XMP-dc:Description`, read as Markdown |
...
A caption is Markdown, as the footer line is (R-18): emphasis and links reach the page, raw HTML in it does not. Where the caption is used as text rather than rendered — `alt` (D-9), a page's title when the photograph has none, an `og:description` (F-20) — it carries its words without its markup.
...
**F-5 (MUST)** Each photo gets derivatives at several widths for `srcset` (default 200 / 400 / 800 / 1200 / 1600 / 2400 px long edge — 1200 because a phone at two or three times its CSS width needs about 1000–1100 px, which would otherwise take 1600 — configurable under R-18), and square (1:1) derivatives centre-cropped from it. Squares serve every place that needs a uniform tile and never appear in a photo's own `srcset`. A photo page's `sizes` states the width the layout will actually draw — the column, or the height cap times the photo's aspect ratio, whichever is smaller — since a browser chooses a file from that hint before layout. A square is bounded by the short edge; where a photo is too small for a size, the caller falls back to the uncropped ladder. Nothing is upscaled.
...
**F-7 (MUST)** Album index at `/photos/albums/`, in the site menu (F-21): one section per listed album, a sample grid (F-5) of its first eleven photos in album order — each tile leading into the album, to that photograph's scoped page (F-11a) — on one side of the rail, and its title, year span and intro on the other. The twelfth cell always links into the album, whatever the count. The year span covers the album's published photos — `2024` or `2024–2026`; the album's own `date` still orders the index and the feed (F-19). The front page lists albums plainly, as it lists years, tags and places — a tag as the chip F-11 describes, without its top level — under headings that are not links: the album index, the whole timeline and the tags page are each one link beneath their list.
...
**F-11a (MUST)** The same photo seen inside a listing is its own page beneath that listing — `/photos/albums/<album>/<id>/`, and likewise under a tag, a location, a year and a month — with the listing's context and previous/next within it. A scoped page declares `/photos/p/<id>/` canonical and stays out of the sitemap. *Because* a page with its own address survives being copied and works without JavaScript, which is what an overlay could not do (F-12).
...
**F-24 (SHOULD)** Gear page at `/photos/gear/`, in the menu, under the headings Cameras and Lenses (R-15). Cameras are ordered by name; lenses by the focal length in their name, a zoom at its wide end, a lens without one last. The index's gear chips use the same order. Each item gets a section: a sample grid (F-5) of its most recent photos, each tile leading to that photograph inside the gear tag (F-11a), on one side of the rail, and its name, its R-17 description and its tag chip on the other.

**F-25 (SHOULD)** On a scoped page (F-11a), the pager is three square thumbnails in one row: the previous photograph, this one, and the next. The neighbours stay inside the listing; the middle one is the way out to this photograph's own page (F-11). A listing does not wrap: at its first photograph there is no previous step and at its last no next one. The missing step is not drawn, so the row fills from the left, and each label is aligned by the slot it occupies — left, centred, right — rather than by the step it belongs to. Each label sits under the side it leads to — previous left, "Photo details" centred, next right. The neighbours carry `rel="prev"` and `rel="next"`, and where the browser runs the site's one script the left and right arrow keys follow them; the links work without it, and a modifier held with the key leaves it to the browser. The pager is the whole of the rail's navigation: a scoped page carries no buttons beside it. The page asks the browser to prefetch its neighbours' documents, which is markup rather than script and costs about 7 KB gzipped each; their photographs are not prefetched, being the weight of a page most readers never ask for.

**F-26 (SHOULD)** Tags page at `/photos/tags/`, in the menu: one section per curated tag in slug order, a sample grid (F-5) of its most recent photos, each tile leading to that photograph inside the tag (F-11a), on one side of the rail, and its R-19 description and its chip — F-11's, without the top level — on the other. Gear tags are left out (F-24). The page grants no reachability under §12 criterion 4, which counts a single tag's listing.
...
**D-9 (MUST)** WCAG AA contrast, visible focus, full keyboard navigation of every grid and of the fullscreen view, and non-empty `alt` on every photo: the caption, else the title, else the photo id. `alt` is read aloud, so a caption reaches it as its words without its Markdown. An id in `alt` is a placeholder, not a description — publishing an untitled photo poorly described beats not publishing it, but each one is curation debt.

**D-10 (SHOULD)** The site stands alone, with configurable title, subtitle and imprint (R-18) and short links to other pages. Its footer carries the configured footer line on the left — a copyright notice, say, or else the site's title; inline Markdown, so it can carry a link, with raw HTML dropped — and on the right a credit, "Generated by exposer and Hugo", linking to exposer's repository and to Hugo's website, then the imprint link when one is set, last and after a middot. The imprint is an R-13 page; the configuration names the slug the footer links to.

**D-11 (SHOULD)** On a photo page (F-11, F-11a), clicking the photo shows it in the browser's fullscreen mode, centred and as large as the screen allows without changing its ratio. Space the photo does not fill is the site's own ground, `#FAFAFA` (D-1), not black. Entering and leaving fullscreen works from the keyboard, and the image keeps its `alt` (D-9). No page carries a button for it: the script makes the photograph itself the control — focusable, named, and answering Enter and Space — once it knows the browser can go fullscreen at all. Until then, and where the API is missing, the photograph is a photograph.
