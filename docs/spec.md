...
**R-18 (SHOULD)** The generator's configuration lives in the library at `<library>/_data/exposer.yaml`: base URL, title, subtitle, footer and imprint, the publication gate (R-5) and the derivative ladder (F-5). Without one the build uses defaults; `--config` overrides it, so one library can be published twice with different settings. `exposer init <library>` writes a commented file holding exactly the defaults, and never replaces an existing one — the only file exposer ever writes into a library.
