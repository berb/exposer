...
**D-11 (SHOULD)** On a photo page (F-11, F-11a), clicking the photo shows it in the browser's fullscreen mode, centred and as large as the screen allows without changing its ratio. Space the photo does not fill is the site's own ground, `#FAFAFA` (D-1), not black. Entering and leaving fullscreen works from the keyboard, and the image keeps its `alt` (D-9). Where a page shows a fullscreen button, that button is the keyboard's way in; where it shows none, as F-25's scoped page does, the script makes the photograph itself the control — focusable, named, and answering Enter and Space — once it knows the browser can go fullscreen at all.
