...
**F-25 (SHOULD)** On a scoped page (F-11a), the pager is three square thumbnails in one row: the previous photograph, this one, and the next. The neighbours stay inside the listing; the middle one is the way out to this photograph's own page (F-11). Each label sits under the side it leads to — previous left, "Photo details" centred, next right. The neighbours carry `rel="prev"` and `rel="next"`, and where the browser runs the site's one script the left and right arrow keys follow them; the links work without it, and a modifier held with the key leaves it to the browser. The pager is the whole of the rail's navigation: a scoped page carries no buttons beside it.
