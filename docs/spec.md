...
**D-10 (SHOULD)** The site stands alone, with configurable title, subtitle and imprint (R-18) and short links to other pages. Its footer carries the configured footer line on the left — a copyright notice, say, or else the site's title; inline Markdown, so it can carry a link, with raw HTML dropped — and on the right the imprint link when one is set, then a credit, "Generated by exposer and Hugo", linking to exposer's repository and to Hugo's website. The imprint is an R-13 page; the configuration names the slug the footer links to.
